...
?>
    ...
    <?php $listcssjs = array(
    		...
    		'catalog/view/theme/'.$config->get( 'config_template' ).'/js/jquery-ui.js',
			...
			'catalog/view/theme/'.$config->get( 'config_template' ).'/js/jquery.validate.min.js'
    ); ?>
...
<div class="<?php if($theme_options->get( 'main_layout' ) == 2) { echo 'fixed-body'; } else { echo 'standard-body'; } ?>">
		...
		<header class="header">
			...
			<?php } else { ?> </div> <?php } ?>
				...
				<div class="search tg-search" id="search">
					<input type="text" class="field" name="search" placeholder="<?php if($theme_options->get( 'search_text', $config->get( 'config_language_id' ) ) != '') { echo html_entity_decode($theme_options->get( 'search_text', $config->get( 'config_language_id' ) )); } else { echo 'Поиск'; } ?>" value="" />
				...
				<button class="call-btn" type="button" data-toggle="modal" data-target="#callModal"></button>
					...
					<div class="slogan">Электрокотлы и электроотопление</div>
					...
					<?php echo $cart; ?>
					<div class="check-mob-btn">
						<a href="http://new.elektrotopka.com.ua/index.php?route=checkout/cart" class="button"></a>
					</div>
			</div>
    ...
    <?php 
		...
		<nav class="primary-nav">
			<div class="container">
				<button type="button" class="show-nav"></button>
				<ul class="nav-list">
					<li>
					 <?php if($current_page != 'home'){ ?><a href="<?php echo $home; ?>">Главная</a> <?php } else { ?> <span>Главная</span> <?php } ?>
					...
					<?php } else { ?>
					<li><a href="<?php echo $category['href']; ?>"><span><strong><?php echo $category['name']; ?></strong></span></a>
					<?php } ?>
						<?php if ($category['children']) { ?>
						<?php 
							$width = '100%';
					...
					<li><a href="/blog/blog.html">Наши новости</a></li>
					<li><a href="/kontaktnaya-informatsiya-i7.html">Контактная информация</a></li>
				</ul>
			</div>
		</nav>	
		<?php } ?>
